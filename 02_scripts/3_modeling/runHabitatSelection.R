...
plotStats <- function(x=NULL, xlab=NULL, season=NULL) {
  ...
  p = ggplot(data) +
    ...
    plot.theme + scale_fill_manual(values=c('#D01C8B', '#4DAC26')) 
...
data = ssf.df
# levels(data$cover_class) <- c('cropland', 'bare/water', 'open', 
# 'sparse', 'closed')
hsf <- glm(case_ ~ hillslope + evi + waterdist + cover_class, 
           data = data, weight = w, family = binomial)
