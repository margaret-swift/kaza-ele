...
Rcpp::List cpp_abm_simulate(
  ...
  std::cout << "LOCATION: cpp_abm_simulate.cpp L129\n\n";
  ...
  for(int i = 1, a = nopt, desi = 0; i < timesteps; i++){
    ...
    while (j < nopt) {
      if(j == 0){ // repeat for each start of each step
        ...
        step_OptionsAll[a] = i;
        j++; a++;
        continue;// this runs the WHILE loop again!!! must increment before.
      }
      ...
      // BOOKMARK!!!!!
      // an IF to make sure the animal only steps through the fence with probability
      //   P (for now this is implicitly zero, but later a P can be passed thru
      //   the function call)
      ...
      //   Eventually, P should be a product of the animal's base willingness
      //   to cross a fence (W) and the fence's condition at that section (C).
      //   Should this possibly also depend on the current behavior? maybe eles
      //   in transit are more likely to cross, like the roan were?
      if (intersect) {
        std::cout << "Target crosses fence. Skipping this one.\n";
        // FAILSAFE!
        if (trialflag < 5000) {
          trialflag++; // increase number of trials so we don't spin out.
          continue;
        } else {
          std::cout << "WARNING ele cannot find target that doesn't cross " <<
            "the fence! Aborting selection...";
        }
      }
      std::cout << "Does not cross fence. Continuing.\n";
